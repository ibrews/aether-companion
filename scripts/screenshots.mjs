...
// --- Transparent mode (clean, no chat) ---
const transparentPage = await browser.newPage();
await transparentPage.setViewport({ width: 1280, height: 720, deviceScaleFactor: 2 });
await transparentPage.goto(BASE_URL, { waitUntil: 'networkidle0' });
await setTimeout(4000);
await transparentPage.click('#bg-toggle');
await setTimeout(1500);
await transparentPage.screenshot({ path: `${SCREENSHOTS_DIR}transparent.png` });
